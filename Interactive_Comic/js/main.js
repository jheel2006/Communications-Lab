...
/* Scene 2 | @s2
  ...
  scene3.on('progress', function(ev) {
    if (ev.scrollDirection == "REVERSE") {
      $('.section3-text').removeClass("animated");
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  // Get the grc-section-2 element
  const grcSection2 = document.querySelector(".grc-section-2");

  // Get the fade-in-picture element
  const fadeInPicture = document.querySelector(".fade-in-picture");

  // Add scroll event listener
  window.addEventListener("scroll", function () {
    // Get the bottom position of grc-section-2
    const grcSection2Bottom = grcSection2.getBoundingClientRect().bottom;

    // Check if the user has scrolled past grc-section-2
    if (grcSection2Bottom <= 0) {
      // Add a class to trigger the fade-in effect
      fadeInPicture.classList.add("fade-in");
    }
  });
});
...
TweenMax.set('.grc-section-6-building-1', {
  y: '20%'
});

TweenMax.set('.grc-section-6-building-2', {
  y: '20%'
});
...
TweenMax.set('.grc-section-6-text', {
  opacity: 0,
  y: '20%'
});

var s6GraphWidth = 400 * 6;
var s6SteppedEase = new SteppedEase(6);
var s6Building1 = $('.grc-section-6-building-1');
var s6Building2 = $('.grc-section-6-building-2');
var s6Char = $('.grc-section-6-char');

var scene6_animation_bg = new TimelineMax()
  .add([
    TweenMax.to(s5Building1, 3, {
      scale: 1.15,
      x: '5%',
...
var scene6_animation = new TimelineMax()
  .add([
    TweenMax.to('.grc-section-5-text-warp-animation', 1.5, {
      backgroundPosition: "-" + s5GraphWidth + "px 0px",
      ease: s5SteppedEase
  ...
  .add([
    TweenMax.to('.grc-section-6-text', 3, {
      opacity: 1,
      y: '0%'
    })
  ]);
...
var scene6 = new ScrollScene({
  ...
  .setTween([scene6_animation, scene6_animation_bg])
  .setPin('.grc-section-6');

scene6.on('enter', function() {
  if (!scene6_trigger) {
    $(document).trigger('setScene', {
      scene: 'Scene 6: Warped',
    });
    scene5_trigger = true;
